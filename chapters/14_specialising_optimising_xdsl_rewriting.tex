...
By examination of its emitted bytecode (Listing \ref{listing:bytecode-profiles-op-build-optimised}), we can be confident in the optimality of the specialised implementation, as it only constructs the object and sets required fields, performing no other extraneous logic. % TODO: "we can be confident in the optimality of the specialised implementation" is a strong claim. Perhaps "near-optimal" or "highly optimised" would be more appropriate, or clarify the scope of "optimality" (e.g., optimal within the constraints of Python's runtime for this specific workload).
