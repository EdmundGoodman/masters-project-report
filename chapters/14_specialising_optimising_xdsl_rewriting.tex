...
We can quantify one cause of this improvement as the specialised code executing fewer, faster bytecode instructions (Listing \ref{listing:bytecode-profiles-op-build-original}, Listing \ref{listing:bytecode-profiles-op-build-optimised}) by leveraging our novel bytecode profiling tool.
...
\ac{mlir}'s implementation is complicated by the fact that its pattern rewriting infrastructure implements constant folding by default. To mitigate this, we modify its implementation to ensure that only the constant folding pattern is correctly exercised.
% Link
% Unlike previous micro-benchmarks, MLIR's implementation of pattern rewriting also introduces non-negligible implementation overhead. As such, we provide a matching implementation of this algorithm in MLIR to guarantee fair comparison.
...
% Argument
Despite this, the specialisation process reamins unchanged
% Link
...
% Link
This information further tells us...
% Argument
% Link
...
% Argument
However, some are more generic to any use case of xDSL, and as such can be applied as generic optimisations. Unfortunately, xDSL is a large codebase, over $150,000$ lines of Python, and as such it is out of scope of this thesis to improve the performance of all of it.
...
% Link
This optimisation was applied in \href{https://github.com/xdslproject/xdsl/pull/4242}{PR \#4242}, resulting in a $x\times$ speedup on micro-benchmarks and $y\times$ speedup on workload z. % TODO: Complete
...
% Argument
% Link
This optimisation was applied in \href{https://github.com/xdslproject/xdsl/pull/4163}{PR \#4163}, resulting in a $x\times$ speedup on micro-benchmarks and $y\times$ speedup on workload z. % TODO: Complete
