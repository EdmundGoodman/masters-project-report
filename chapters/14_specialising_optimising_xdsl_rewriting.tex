...
% Hook
In this chapter, we meet this disentangle these effects by specialising the xDSL framework to implement only a single workload.
% Argument
To do this, we manually identify and remove computation performed by the framework which is unnecessary to its execution of this workload. For example, the framework may calculate and check values that are known as runtime invariants for the selected workload. Checking properties of empty fields is one such example. Since this checking does not contribute to the workload's behavior, it can be removed. Another example is functionality which improves the expressivity of the framework's API, such as runtime type checks to support polymorphism of arguments. Since only a single facet of this API is used by the workload, this can also be removed.
...
% TODO: Does this want to be lifted into the dynamism section?
% Hook
% Argument
We use \texttt{valgrind}'s \texttt{callgrind} tool \cite{valgrindtmdevelopersCallgrindCallgraphGenerating} to instrument \ac{mlir}'s C++ implementation, as opposed to ByteSight for xDSL's Python.
For the same implementation, Python dispatches only $35$ bytecode instructions in comparison with AArch64 assembly taking $708$ (\autoref{fig:ubenchmark-original-trait-performance}).
This demonstrates... % TODO:...
% Link
...
% Hook
In order to draw fair comparisons between the two frameworks, their implementations of the constant folding pattern rewriting must be equivalent.
% Argument
...
% Link
Whilst our constant folding pass is less complex than existing passes such as canonicalisation, it is still much more complex than any of the above micro-benchmarks. As such, there are many more opportunities for the specialisation of its implementation.
% Argument
Despite this, the specialisation process remains unchanged.
Applying a tracing profiler, we can identify computation which can be elided (\autoref{fig:constant-fold-original-viztracer}).
As with the micro-benchmarks, this includes functionality to increase the expressiveness of the API, and logic which is unrelated to the workload. Furthermore, we apply function inlining to avoid the performance cost of interacting with the call stack.
% Link
This yields a specialised implementation with reduced overhead, which
...
%% TODO: Some interesting implementation details???
% Link
% Argument
% Link
...
% Link
From this specialisation process, we gained...
% Argument
% Link
...
% Link
From this specialisation process, we lost
% Argument
% Further weaknesses of workload so large hard to reason about and MLIR itself has implementation details adding overhead -- but does demonstrate xDSL can be improved and lower bound C++ performance
% Link
...
Despite this, we demonstate that the specialisation process lead to generalisable optimisations with measurable performance impact.
Through this, we enable performance improvement over time, which can be quantified using our previosuly introduced measurement infrastructure (\autoref{ssec:infrastructure}).
% Link
...
% Argument
We find that ..
% something something generalising optimisations
In addition to this, for some workloads we find that Python's performance can further approach C++
%
In the next chapters, we leverage this best-case performance specialisatgion as a baseline to examine the effect of both recent improvements to CPython's runtime, and the cost of dynamism in comparison with statcially compiled languages.
