...
% Link
This motivates the development of shared compiler infrastructure which facilitates development velocity, for example by having short build times, a simple syntax, and a convenient API.
...
% Argument
After this, the \ac{gcc}  project introduced a shared infrastructure for compiling several languages, including C, C++, and FORTRAN, but its monolithic and tightly-coupled design limited the reusability of its components \cite{stallmanUsingGnuCompiler2009}.
% Link
...
% Argument
LLVM productised a collection of good ideas in compiler research including \ac{ssa},  inter-procedural analysis, and a low-level \ac{ir} into a convenient framework.
% SSA
% LLVM IR
% Transformation passes
These desirable properties lead to LLVM's widespread adoption, being used as a backend for many languages including C++, Rust, and Swift.
However, the demands of modern workloads present a number of challenges for LLVM.
% Link
For example, LLVM's low-level \ac{ir} is not well suited to the high-level optimisations required for \ac{dag} representations of machine learning workloads. In addition to this key infrastructure such as parsing and printing must be implemented by hand, slowing development velocity.
...
% TODO: Probably need pattern rewriting, can get away without irdl
% % \subsection{Pattern rewriting optimisations}
% % \label{ssec:pattern-rewriting}
% % Hook
% Pattern rewriting is a key idiom in \ac{mlir}.
% % Argument
% % Link

% % \subsection{IR definition language (IRDL)}
% % \label{ssec:irdl}
% % Hook
% \ac{irdl} \cite{fehrIRDLIRDefinition2022a}.
% % Argument
% % Link
...
% Argument
It prioritises developer productivity through Python's easy-to-use syntax and fast build times, instead sacrificing some runtime performance provided by C++. This makes it suitable for teaching and
% It leverages \ac{irdl} to provide an intuitive API for defining operations in Python's object model.
As a result of its similar data structures and shared textual \ac{ir}, xDSL can be used as a side-kick compiler for \ac{mlir}, facilitating modular replacement of individual components of its pipeline.
% Link
...
% Hook
The Python language has many implementations, with the canonical reference implementation CPython being...
% Argument
% Possibly something about JIT compilers like PyPy and Faster CPython, and something about PyPerformance benchmarks.
% Link
...
% Argument
% Link
In their paper ``Copy-and-Patch Compilation'', Xu an Kjolstad present a novel approach to avoid this runtime cost \cite{xuCopyandpatchCompilationFast2021a}.
